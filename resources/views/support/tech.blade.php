...
<div class="container">
        ...
        <div class="ticket-card">
            ...
            <div class="ticket-header">
                <div>
                    ...
                    @php
                        ...
                        if ($status === 'answered') {
                            ...
                            $badgeText = 'Ожидается ответ';
                        } elseif ($status === 'closed') {
                            $badgeClass = 'badge-gray';
                            $badgeText = 'Закрыт';
                        } else {
                            $badgeClass = 'badge-green';
                            $badgeText = 'Ответ получен';
                        }
                    @endphp
                ...
                <div class="ticket-actions">
                    <form method="POST"
                        action="{{ $current_ticket->status === 'closed' ? route('support.tech.reopen', $current_ticket->id) : route('support.tech.close', $current_ticket->id) }}">
                        @csrf
                        <input type="hidden" name="ticket_id" value="{{ $current_ticket->id }}">
                        <button
                            class="btn btn-gray">{{ $current_ticket->status === 'closed' ? 'Открыть' : 'Закрыть' }}</button>
                    </form>
                            ...
                            @if(!empty($msg['file']))
                                ...
                                <div class="chat-file">
                                    @if(in_array(strtolower($ext), ['jpg', 'jpeg', 'png']))
                                        ...
                                        <a href="{{ Storage::url($msg['file']) }}" target="_blank" class="chat-link">📎
                                            {{ basename($msg['file']) }}</a>
                                    @else
                                        <a href="{{ Storage::url($msg['file']) }}" target="_blank" class="chat-link">📎
                                            {{ basename($msg['file']) }}</a>
            ...
            <div class="chat-form">
                <form method="POST" enctype="multipart/form-data" action="{{ route('support.tech.send') }}">
                    @csrf
                    <input type="hidden" name="ticket_id" value="{{ $current_ticket->id }}">
                    <textarea name="message" placeholder="Напишите ответ агенту..." rows="2" class="input-text"></textarea>
    ...
    @else
        ...
        <div class="ticket-search">
            <input type="text" id="search" placeholder="🔍 Поиск по ID или email..." value="{{ $search }}"
                class="input-text">
            <button onclick="searchTickets()" class="btn btn-gray">Найти</button>
        </div>

        <div class="ticket-tabs">
            <a href="{{ route('support.tech', array_merge(request()->all(), ['tab' => 'active', 'sort' => 'answered'])) }}"
                class="{{ ($tab === 'active' && $sort === 'answered') ? 'tab-active' : 'tab' }}">Answered first</a>
            <a href="{{ route('support.tech', array_merge(request()->all(), ['tab' => 'active', 'sort' => 'date'])) }}"
                class="{{ ($tab === 'active' && $sort === 'date') ? 'tab-active' : 'tab' }}">По дате</a>
            <a href="{{ route('support.tech', array_merge(request()->all(), ['tab' => 'archive'])) }}"
                class="{{ ($tab === 'archive') ? 'tab-active' : 'tab' }}">📁 Архив</a>
        </div>
